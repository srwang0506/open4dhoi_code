...
def compute_mask_loss(width, height, video_dir, hverts, overts, hfaces, ofaces, mask_weight=1.5, edge_weight=1e-3, frame_idx=None):
    ...
    renderer = nr.renderer.Renderer(
        ...
        R=R,
        t=T,
        orig_size=(width, height),  # Use max(width, height) to match image_size
        anti_aliasing=True,
    )
    ...
    if (i==0):
        ...
        mask = (mask * 255).astype('uint8')  # Convert to 8-bit pixels
    ...
    gt_paths = {  
        'obj': os.path.join(video_dir, 'mask_dir', f"{str(i).zfill(5)}.png"),
        'human': os.path.join(video_dir, 'human_mask_dir', f"{str(i).zfill(5)}.png")  
    }
    render_masks = {  
        'obj': object_mask,
        'human': human_mask

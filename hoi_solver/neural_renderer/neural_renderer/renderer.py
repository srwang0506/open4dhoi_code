...
from .rasterize import rasterize, rasterize_rgbad, rasterize_silhouettes, rasterize_depth
from .vertices_to_faces import vertices_to_faces

# Create a namespace object so existing `nr.xxx` calls work without changes
class _NR:
    lighting = staticmethod(lighting)
    look = staticmethod(look)
    look_at = staticmethod(look_at)
    perspective = staticmethod(perspective)
    projection = staticmethod(projection)
    rasterize = staticmethod(rasterize)
    rasterize_rgbad = staticmethod(rasterize_rgbad)
    rasterize_silhouettes = staticmethod(rasterize_silhouettes)
    rasterize_depth = staticmethod(rasterize_depth)
    vertices_to_faces = staticmethod(vertices_to_faces)
